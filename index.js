const {
 ...
 teks = `Sayonara @ $ {num.split ('@') [0]} tchau ja foi tarde 😂👋`
 let buff = await getBuffer (ppimg)
 client.sendMessage (mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"givenJid": [num]}})
 }
 } catch (e) {
 ...
 sucesso: '✔️ Pronto ✔️',
 erro: {
 vara: '❌ Falha, ocorreu um erro adesivo ao converter a imagem para ❌',
 Iv: '❌ O link não é válido ❌'
 },
 só: {
 grupo: '❌ Este comando só pode ser usado em grupos!  ❌ ',
 ownerG: '❌ Esse comando só pode ser usado por grupo de proprietários!  ❌ ',
 ...
 const ownerNumber = ["6285892766102@s.whatsapp.net"] // substitua pelo seu número
 ...
 case 'tp':
 if (args.length <1) {
 resposta de retorno ('Pilih themenya om, 1 - 162')
 } else if (args [0] .toLowerCase () === 'list') {
 teks = await fetchText ('https://mhankbarbar.tech/api/textpro/listtheme')
 teks = teks.replace (/ <br> / g, '\ n')
 resposta de retorno (teks)
 } else if (args.length <2) {
 resposta de retorno ('Teksnya juga dong om')
 }
 responder (mess.wait)
 anu = `https://mhankbarbar.tech/api/textpro?pack=${args[0]}&text=${body.slice(3+args[0].length+1)}&apiKey=${apiKey}  `
 voss = aguarda busca (anu)
 ftype = require ('tipo de arquivo')
 vuss = await ftype.fromStream (voss.body)
 //console.log(vuss)
 if (vuss! == undefined) {
 client.sendMessage (from, await getBuffer (anu), image, {caption: mess.success, quoted: mek})
 } outro {
 resposta ('Terjadi kesalahan, silahkan pilih tema deitado')
 }
 pausa
 case 'ep':
 if (args.length <1) {
 resposta de retorno ('Pilih themenya om, 1 - 216')
 } else if (args [0] .toLowerCase () === 'list') {
 ...
 responder ('Ativar com sucesso o modo simi neste grupo ✔️')
 } else if (Number (args [0]) === 0) {
 samih.splice (de, 1)
 fs.writeFileSync ('./ src / simi.json', JSON.stringify (samih))
 responder ('Desativando o modo simi com sucesso neste grupo ✔️')
 } outro {
 ...
 responder ('Ativou com sucesso o recurso de boas-vindas neste grupo ✔️')
 } else if (Number (args [0]) === 0) {
 welkom.splice (de, 1)
 fs.writeFileSync ('./ src / welkom.json', JSON.stringify (welkom))
 responder ('Desativado com sucesso o recurso de boas-vindas neste grupo ✔️')
 } outro {
 responder ('1 untuk mengaktifkan, 0 untuk menonaktifkan')
 }
                                       pausa
 ...
 menções (`Foto profile Berhasil di perbarui menggunakan foto profile @ $ {id.split ('@') [0]}`, [jid], verdadeiro)
 } catch (e) {
 responder ('Gagal om')
 }
 pausa
